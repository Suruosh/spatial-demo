...
// Volumetric action button. 3D feel via CSS scale (no animation library).
export function Button3D({ children, variant = 'white', className = '', ...rest }: Button3DProps) {
  return (
    <button
      style={{ transformStyle: 'preserve-3d' }}
      className={`relative flex items-center justify-center font-bold uppercase text-[10px] tracking-widest px-8 py-3 transition-transform duration-200 hover:scale-105 active:scale-95 ${VARIANTS[variant]} ${className}`}
      {...rest}
    >
      <span className="absolute inset-0 border-t border-white/20 pointer-events-none" />
      {children}
    </button>
  );
}
